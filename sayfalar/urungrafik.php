...
<div class="col-xl-6 col-sm-12 mb-3">	
	...
	<?php       
        try
        {
				 ...
				 echo '<option value="/index.php?sayfa=urungrafik&yil=">Yıl seçiniz</option>';
             while ( $row = $sonuckod->fetch() )
             {
                echo '<option value="/index.php?sayfa=urungrafik&yil='.$row['yil'].'">'.$row['yil'].'</option>'; 
             }
             echo '</select>';
            }
...
<div class="col-lg-6">
                                <div class="card mb-4">
                                    <div class="card-header">
                                        ...
                                        Yıllara Göre Toplam Satış (Adet)
                                    </div>
...
$satis_sorgu = mysqli_query($baglanti,"SELECT count(satislar.urun_id) as toplamsatis FROM satislar WHERE satislar.yil = '$yil' GROUP BY satislar.urun_id ");
$urun_sorgu = mysqli_query($baglanti,"SELECT urunbaslik FROM urunler GROUP BY urunbaslik");
?>
...
<script>
					...
					var adlar=[<?php while ($sonuc=mysqli_fetch_assoc($urun_sorgu)) { echo '"' . $sonuc['urunbaslik'] . '",';}?>];
