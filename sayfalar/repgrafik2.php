...
<div class="col-xl-6 col-sm-12 mb-3">	
	...
	<?php       
        try
        {
				 ...
				 echo '<option value="/index.php?sayfa=repgrafik2&yil=">Yıl seçiniz</option>';
             while ( $row = $sonuckod->fetch() )
             {
                echo '<option value="/index.php?sayfa=repgrafik2&yil='.$row['yil'].'">'.$row['yil'].'</option>'; 
             }
             echo '</select>';
            }
...
<div class="col-lg-10">
                                <div class="card mb-4">
                                    <div class="card-header">
                                        ...
                                        Reprezanta Göre Toplam Satış (Adet)
                                    </div>
...
$satis_sorgu = mysqli_query($baglanti,"SELECT count(satislar.rep_id) as toplamadet FROM satislar WHERE satislar.yil = '$yil' GROUP BY satislar.rep_id");
...
<script>	
                   	var miktar=[<?php while ($sonuc2=mysqli_fetch_assoc($satis_sorgu)) { echo '"' . $sonuc2['toplamadet'] . '",';}?>];	
